...
- `audio-server/` contains the Janus Gateway AudioBridge configuration used by the Render Docker service.
...
The Janus media-server configuration lives in `audio-server/`. On Render, the root `Dockerfile` runs the Node backend, Janus Gateway, and nginx in the same web service. nginx routes `/janus` to Janus WebSocket transport and routes the API/Socket.IO traffic to the Node backend. Rooms are created dynamically from the WatchParty room code, and clients publish Opus microphone audio plus capturable watch-player audio into the same AudioBridge room.
...
TURN may be required for production reliability, especially for mobile, restrictive NAT, corporate networks, and hosted environments that do not expose UDP media ports cleanly. Render is useful for deployment and testing, but the free tier may not be ideal for low-latency WebRTC audio. For a single Render service like `https://watchparty-6a3e.onrender.com`, the frontend Janus URL is `wss://watchparty-6a3e.onrender.com/janus`.
